    ...
    <>
      ...
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 sm:px-8">
          ...
          >
            {company.name}
            <span className={compact ? "text-copper-dark" : "text-copper-light"}>.</span>
          </Link>
          ...
          <div className="flex items-center gap-4">
            ...
            <Link
              href="/kontakt"
              className="hidden rounded-full bg-copper px-5 py-2.5 text-sm font-semibold text-paper transition hover:bg-copper-light sm:block"
            >
              Kontakt aufnehmen
            </Link>
            <button
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[60] flex flex-col bg-slate text-paper lg:hidden"
          >
              ...
              {navPages.map((page, i) => (
                ...
                >
                  <Link
                    href={page.href}
                    onClick={() => setMenuOpen(false)}
                    className="font-display text-3xl font-medium text-paper/90 transition hover:text-copper-light"
                  >
                    <span className="mr-3 text-sm text-copper-light">{page.number}</span>
                    {page.label}
                  </Link>
                </motion.div>
              ))}
